...
COMMIT_TIMESTAMP=$(git log -1 --date=iso-strict --format="%ad" 2>/dev/null || printf '')
GIT_CONFIG_VARIABLES="APTIBLE_GIT_COMMIT_SHA=$COMMIT_SHA APTIBLE_GIT_REPOSITORY_URL=$REPO_URL APTIBLE_GIT_REF=$BRANCH APTIBLE_GIT_COMMIT_URL=$COMMIT_URL APTIBLE_GIT_COMMIT_TIMESTAMP=$COMMIT_TIMESTAMP"
...
if [ "$INPUT_TYPE" == "git" ]; then
  if [ "$INPUT_GIT_REMOTE" == "primetime.aptible.com" ]; then
    # for production we want to ensure host key doesn't change from these
    ...
    # for development we want to accept the host key, essentially skipping this check
    ssh-keyscan -p 43022 "$INPUT_GIT_REMOTE" >> ./known_hosts
  fi
